...
import { Plus, Check, Play, Square, ChevronDown, ChevronUp, Trash2, Calendar as CalendarIcon, Clock, Repeat } from 'lucide-react';
...
export function TasksView() {
  ...
  const addTask = async (e: React.FormEvent) => {
    ...
    const parsedMinutes = parseInt(targetDurationMinutes);
    if (!isNaN(parsedMinutes) && parsedMinutes > 0) {
      duration = parsedMinutes * 60; // convert to seconds
    }

    let parsedDate: Date | undefined = undefined;
    if (dueDate) {
      parsedDate = new Date(dueDate);
      // Adjust timezone offset so it doesn't shift
  ...
  return (
    <div className="max-w-4xl mx-auto h-full flex flex-col">
      ...
      <form onSubmit={addTask} className="glass-card p-4 mb-8 flex flex-col gap-4">
...
function TaskItem({ task }: { task: Task }) {
  const { activeTaskId, targetDuration, startTimer, stopTimer, elapsedSeconds } = useTimer();
      ...
      <div className="p-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpanded(!expanded)}>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-1">
            ...
            {task.targetDuration && (
              <span className="px-2 py-0.5 rounded-full text-xs border border-emerald-500/50 text-emerald-400 flex items-center gap-1">
                <Clock className="w-3 h-3" /> {Math.floor(task.targetDuration / 60)}m
              </span>
            )}
            {task.subtasks.length > 0 && (
              <span className="text-xs text-gray-500 ml-1">{task.subtasks.filter(st => st.completed).length}/{task.subtasks.length} subtasks</span>
            )}
          </div>
        </div>
          ...
          {isActive ? (
            <div className="flex items-center gap-3 bg-accent/10 px-3 py-1.5 rounded-lg border border-accent/20">
              <span className="text-accent font-mono text-sm">
                {targetDuration !== null ? formatTime(displaySeconds) : formatTime(elapsedSeconds)}
              </span>
            ...
            <button 
              onClick={() => startTimer(task.id, task.targetDuration)}
              disabled={task.completed}
              title={task.targetDuration ? "Start Timer" : "Start Stopwatch"}
              className={cn(
                "w-10 h-10 rounded-full flex items-center justify-center transition-colors disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-gray-400 border",
                task.targetDuration 
                  ? "border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 hover:border-emerald-400" 
                  : "border-transparent text-gray-400 hover:text-accent hover:bg-accent/10"
              )}
            >
